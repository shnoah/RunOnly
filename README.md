...
설정의 `심박 존` 화면에서는 PNR 자동 기준, 최대심박 기준, 러닝 역치 기준, 수동 bpm 범위를 선택할 수 있습니다. 상세 화면의 심박 분포는 현재 선택한 기준과 bpm 범위를 함께 보여줍니다.
...
The `Heart Rate Zones` setting supports PNR automatic zones, max-heart-rate zones, running LTHR zones, and manual bpm ranges. Run details show the active zone basis and bpm ranges alongside the heart rate distribution.
